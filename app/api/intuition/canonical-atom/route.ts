  ...
  return (candidate.term?.vaults ?? []).reduce(
    (best, vault) => {
      ...
      if (
        ...
        return { positionCount: vault.position_count, totalShares: shares };
      }

      return best;
    },
    { positionCount: 0, totalShares: 0n },
  );
}
...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(config.graphqlUrl, {
    ...
    body: JSON.stringify({
      ...
      variables: { label, limit: 10 },
    }),
    cache: 'no-store',
  });

  if (!response.ok) {
    return NextResponse.json({ error: `Canonical atom lookup failed with HTTP ${response.status}.` }, { status: 502 });
  }
  ...
  const bestCandidate = (payload.data?.atoms ?? [])
    ...
    .sort((left, right) => {
      ...
      if (right.type !== left.type) {
        if (right.type !== 'TextObject' && left.type === 'TextObject') return 1;
        if (left.type !== 'TextObject' && right.type === 'TextObject') return -1;
      }
