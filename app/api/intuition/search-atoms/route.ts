...
function getBestVaultSignal(candidate: GraphAtomCandidate): { positionCount: number; totalShares: bigint } {
  return (candidate.term?.vaults ?? []).reduce(
    (best, vault) => {
      ...
      if (
        ...
        return { positionCount: vault.position_count, totalShares };
      }
...
export async function GET(request: NextRequest) {
  ...
  if (!response.ok) {
    return NextResponse.json({ error: `Atom search failed with HTTP ${response.status}.` }, { status: 502 });
  }
